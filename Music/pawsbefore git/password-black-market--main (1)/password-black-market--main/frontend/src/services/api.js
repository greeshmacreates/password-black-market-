...
// Legacy compatibility for old ClueMarket screens.
// Returns a flat clues array with _id/category/price shape.
export const getClues = async () => {
  const accountsRes = await getAccounts();
  const clues = (accountsRes.data || []).flatMap((account) =>
    (account.clues || []).map((clue) => ({
      _id: clue.clueId,
      category: clue.category,
      price: Number(clue.cost ?? 0),
